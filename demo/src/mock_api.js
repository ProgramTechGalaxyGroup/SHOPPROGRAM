...
(function () {
  ...
  window.fetch = async function (url, options = {}) {
    ...
    if (path === "/api/auth/me") {
      ...
      if (userJson) {
        return makeResponse({ ok: true, user: JSON.parse(userJson) });
      }
    ...
    if (path === "/api/shifts/active" && method === "GET") {
      ...
      if (activeShift) {
        return makeResponse({ ok: true, shift: JSON.parse(activeShift) });
      }
    ...
    if (path === "/api/shifts/start" && method === "POST") {
      ...
      const user = JSON.parse(localStorage.getItem("demo_session_user") || "{}");
    ...
    if (path === "/api/shifts/end" && method === "POST") {
      ...
      const activeShift = JSON.parse(localStorage.getItem("demo_active_shift") || "{}");
      ...
      const sales = JSON.parse(localStorage.getItem("demo_db_sales") || "[]");
      ...
      const shiftsLog = JSON.parse(localStorage.getItem("demo_db_shifts_history") || "[]");
    ...
    if (path === "/api/sync/pull") {
      const products = JSON.parse(localStorage.getItem("demo_db_products") || "[]");
      const components = JSON.parse(localStorage.getItem("demo_db_components") || "[]");
      const categories = JSON.parse(localStorage.getItem("demo_db_categories") || "[]");
      const sales = JSON.parse(localStorage.getItem("demo_db_sales") || "[]");
    ...
    if (path === "/api/sales" && method === "POST") {
      ...
      body.items.forEach(it => {
        ...
        if (pInfo && pInfo.inventory_mode === "recipe" && pInfo.component_ids) {
          try {
            const comps = JSON.parse(pInfo.component_ids);
            comps.forEach(c => {
